...
| Toolchain | AGP 9.2.1 · Kotlin built-in + KGP 2.2.20 (settings classpath + buildscript) · Gradle 9.4.1 · Java 17 | `android/settings.gradle.kts` |
...
| AGP / Gradle / Kotlin | 9.2.1 / 9.4.1 / built-in Kotlin with KGP 2.2.20 (settings classpath `apply false` + buildscript; `android.builtInKotlin=true`; AGP legacy-DSL compat `android.newDsl=false` until the Flutter tool finishes its new-DSL migration) |
| Java | 17 |
| versionName / versionCode | `0.34.23` / `63` (in `pubspec.yaml`) |
